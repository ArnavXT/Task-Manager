...
def emit_task_event(event_name, task_data):
    if current_user.is_authenticated:
        socketio.emit(event_name, task_data, namespace='/tasks', room=f'user_{current_user.id}')
...
def get_tasks():
    ...
    query = query.order_by(sort_col.asc() if order ==
                           'asc' else sort_col.desc())
    all_tasks = query.all()
...
def create_task():
    ...
    if not data:
        return jsonify(
            {'success': False, 'message': 'Request body must be JSON.'}), 400
    title = data.get('title', '').strip()
    if not title:
        return jsonify(
            {'success': False, 'message': 'Title is required.'}), 400
    ...
    if priority not in Task.PRIORITY_CHOICES:
        return jsonify(
            {'success': False, 'message': f'Invalid priority. Choose from: {Task.PRIORITY_CHOICES}'}), 400
    if status not in Task.STATUS_CHOICES:
        return jsonify(
            {'success': False, 'message': f'Invalid status. Choose from: {Task.STATUS_CHOICES}'}), 400
    due_date = None
    if data.get('due_date'):
        try:
            ...
            return jsonify(
                {'success': False, 'message': 'Invalid due_date format.'}), 400
    ...
    emit_task_event(
        'task_created', {
            'message': f'New task created: {
                task.title}', 'task': task_dict, 'user_id': current_user.id})
    return jsonify(
        {'success': True, 'message': 'Task created.', 'task': task_dict}), 201
...
def update_task(task_id):
    ...
    if 'title' in data:
        ...
        if not t:
            return jsonify(
                {'success': False, 'message': 'Title cannot be empty.'}), 400
        task.title = t
    ...
    emit_task_event(
        'task_updated', {
            'message': f'Task updated: {
                task.title}', 'task': task_dict, 'user_id': current_user.id})
    return jsonify(
        {'success': True, 'message': 'Task updated.', 'task': task_dict}), 200
    ...
    emit_task_event(
        'task_deleted', {
            'message': f'Task deleted: {
                task_info["title"]}', 'task': task_info, 'user_id': current_user.id})
...
def bulk_update_status():
    ...
    if not task_ids or not new_status:
        return jsonify(
            {'success': False, 'message': 'task_ids and status are required.'}), 400
    ...
    db.session.commit()
    emit_task_event('bulk_updated',
                    {'message': f'{updated} tasks marked as {new_status}',
                     'count': updated,
                     'user_id': current_user.id})
    return jsonify(
        {'success': True, 'message': f'{updated} tasks updated to {new_status}.'}), 200
